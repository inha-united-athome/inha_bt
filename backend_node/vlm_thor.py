...
IMAGE_TOPIC = "/camera/camera_head/color/image_raw/compressed"   # 필요하면 여기만 바꾸면 됨
IMAGE_TOPIC_RIGHT = "/camera/camera_right/color/image_raw/compressed"
...
class VLMActionServer(Node):
    ...
    - mode 4: right live image(compressed) + prompt (+ optional text) -> LLM vision
    """
    ...
    def __init__(self):
        ...
        # ---- right image (CompressedImage) ----
        self.latest_right_image_bgr = None
        self.last_right_img_stamp = None
        self.last_right_img_time = 0.0
        self.last_right_img_bytes = 0
        self.last_right_img_format = ""
        self.right_img_count = 0
        self._right_fps_t0 = time.time()
        self._right_fps_cnt = 0
        self.right_img_fps = 0.0
        ...
        self.get_logger().info(f"[VLM] Subscribing: {IMAGE_TOPIC_RIGHT} (CompressedImage, sensor_data QoS)")
    ...
    def timer_debug(self):
        ...
        right_age_ms = (now - self.last_right_img_time) * 1000.0 if self.last_right_img_time > 0 else -1.0

        msg = String()
        msg.data = (
            f"img_count={self.img_count} fps={self.img_fps:.1f} "
            f"age_ms={age_ms:.0f} bytes={self.last_img_bytes} fmt='{self.last_img_format}' "
            f"| right_count={self.right_img_count} right_fps={self.right_img_fps:.1f} "
            f"right_age_ms={right_age_ms:.0f} right_bytes={self.last_right_img_bytes} "
            f"right_fmt='{self.last_right_img_format}'"
        )
    ...
    def image_right_cb(self, msg: CompressedImage):
        try:
            self.right_img_count += 1
            self.last_right_img_stamp = msg.header.stamp
            self.last_right_img_time = time.time()
            self.last_right_img_bytes = len(msg.data)
            self.last_right_img_format = msg.format or ""

            self._right_fps_cnt += 1
            dt = self.last_right_img_time - self._right_fps_t0
            if dt >= 1.0:
                self.right_img_fps = self._right_fps_cnt / dt
                self._right_fps_cnt = 0
                self._right_fps_t0 = self.last_right_img_time

            self.latest_right_image_bgr = self.bridge.compressed_imgmsg_to_cv2(
                msg, desired_encoding="bgr8"
            )

        except Exception as e:
            self.get_logger().warn(f"[VLM] right image convert failed: {e}")
    ...
    def execute_cb(self, goal_handle):
        ...
        try:
            ...
            if mode == 1:
                ...
                fb.state, fb.progress = "mode4_wait_image", 0.2
                goal_handle.publish_feedback(fb)

                wait_ms = 3000
                t0 = time.time()
                while self.latest_right_image_bgr is None:
                    check_cancel_timeout()
                    if (time.time() - t0) * 1000.0 > wait_ms:
                        raise RuntimeError(
                            f"no_right_image_received_within_{wait_ms}ms "
                            f"(check topic name/QoS/domain)."
                        )
                    time.sleep(0.02)

                stale_ms = 1500
                age_ms = (time.time() - self.last_right_img_time) * 1000.0
                if age_ms > stale_ms:
                    raise RuntimeError(
                        f"right_image_is_stale age_ms={age_ms:.0f} > {stale_ms}ms "
                        f"(publisher stopped? network? QoS?)"
                    )

                self.get_logger().info(
                    f"[VLM][mode4] got image: age_ms={age_ms:.0f}, bytes={self.last_right_img_bytes}, "
                    f"fmt='{self.last_right_img_format}', fps={self.right_img_fps:.1f}"
                )

                fb.state, fb.progress = "mode4_vision", 0.5
                goal_handle.publish_feedback(fb)

                caption = self.llm_live_image(self.latest_right_image_bgr, prompt, text)
